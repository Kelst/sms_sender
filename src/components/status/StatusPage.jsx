...
export default function StatusPage() {
  ...
  const handleResend = async () => {
    try {
      const resp = await axios.get("https://sms.multiprovider.info/api/sendSms-api-to-user-again");
  ...
  const loadMoreMessages = () => {
    axios.get(`https://sms.multiprovider.info/api/messages?page=${currentPage + 1}`)
      .then(response => {
  ...
  useEffect(() => {
    ...
    const fetchInitialMessages = () => {
      axios.get(`https://sms.multiprovider.info/api/messages?page=1`)
        .then(response => {
    ...
    const fetchTotal = async () => {
      try {
        const total = await axios.get(`https://sms.multiprovider.info/api/totalSMS`);
